...
class BoutiqueController extends AbstractController
{
    ...
    #[Route("/api/boutique/filter", name: "boutique_filter", httpMethods: ["POST"])]
    public function filterProducts()
    {
        if (!is_null($_POST)) {
            ...
            $type = $_POST['type'];
            $statement = 'SELECT * FROM bijoux JOIN images ON images.bijoux_id = bijoux.id WHERE images.principal = 1';

            $conditions = [];

            if (!is_null($pierre) && $pierre != 'null') {
                $conditions[] = "bijoux.pierre_id = " . $pierre;
            }

            if (!is_null($taille) && $taille != 'null') {
                $conditions[] = "bijoux.taille_id = " . $taille;
            }

            if (!is_null($couleur) && $couleur != 'null') {
                $conditions[] = "bijoux.couleur_id = " . $couleur;
            }

            if (!is_null($type) && $type != 'null') {
                $conditions[] = "bijoux.types_id = " . $type;
            }

            if (!empty($conditions)) {
                $statement .= " AND " . implode(" AND ", $conditions);
            }
            //     $statement .= ' AND bijoux.pierre_id = :pierre';
            // if(!is_null($taille))
            //     $statement .= ' AND bijoux.taille_id = :taille';
            // if(!is_null($couleur))
            //     $statement .= ' AND bijoux.couleur_id = :couleur';
            // if(!is_null($type))
            //     $statement .= ' AND bijoux.type_id = :type';

            $stmt = $this->db->prepare($statement);
            $stmt->execute();
            $bijoux = $stmt->fetchAll(PDO::FETCH_ASSOC);
            return $this->returnJsonResponse(['success' => true, 'message' => $bijoux]);
        }

        return $this->returnJsonResponse(['success' => true, 'message' => 'sda']);
    }
    ...
    public function getAllProducts()
    {
        ...
        return $this->twig->render('/Pages/boutique.html.twig', [
            'bijoux' => $bijoux,
            'tailles' => $tailles,
            'couleurs' => $couleurs,
            'types' => $types,
    ...
    public function getProductDetails(int $id)
    {
        ...
        $statement = "SELECT * FROM bijoux JOIN images ON images.bijoux_id = bijoux.id WHERE bijoux.id = :id AND images.principal = 1";
        $stmt = $this->db->prepare($statement);
        $stmt->execute(['id' => $id]);
        ...
        if ($userLoggedIn) {
            ...
            return $authController->renderPage(
    ...
    public function addCart(int $userId, int $productId)
    {
        ...
        if ($userLoggedIn) {
            ...
            if (isset($userCart[$bijoux['id']])) { // Use 'id' key here
                ...
                $userCart[$bijoux['id']]['quantity']++; // Use 'id' key here
            } else {
                // Le produit n'existe pas dans le panier, l'ajouter avec une quantité de 1
                $bijoux['quantity'] = 1; // Add quantity to the product information array
                $userCart[$bijoux['id']] = $bijoux; // Use 'id' key here
            }
